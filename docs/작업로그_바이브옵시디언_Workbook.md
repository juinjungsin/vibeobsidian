...
│   ├─ CLAUDE.md / AGENTS.md                  ← 볼트 운영 규칙 (내용 동일, 7개 절)
│   ├─ README.md                              ← 설치 3단계 안내
│   ├─ 00_Setup/commands/ingest.md, query.md  ← 명령어 원본 (온보딩 시 AI가 .claude/commands로 복사)
│   ├─ 10_Inbox / 20_Raw / 30_Wiki(Concept·Entity·Guide·MOC) / 40_Query / 50_Output
│   └─ 각 폴더에 _README 안내 노트
...
- **폴더 흐름**: 10_Inbox → 20_Raw → 30_Wiki → 40_Query → 50_Output (+ 00_Setup)
...
2. [미결] StarterKit/.claude/ 빈 폴더가 디스크에 잔존 — 탐색기에서 수동 삭제 필요(zip에는 미포함, 영향 없음)
